...
- **큰 사적의 대표점**: 카탈로그 좌표는 원본 폴리곤의 한가운데(polylabel)라서, 면적이 큰 사적은 실제 건물군과 어긋날 수 있다. 사적 "경주 불국사"(39.9만㎡)는 경내가 아니라
  토함산 산림에 찍혀 석굴암과 472m였고, 2026-09-22에 경내(대웅전) 좌표로 보정했다. 면적 10만㎡ 이상 182건(30만㎡ 이상 83건)은 같은 문제가 있을 수 있으나 전수 점검하지 않았다. 국소 고저차 상위 유산은 `terrain-report.mjs top-relief`로 뽑아 눈으로 본다 — 북한산성은 점검해 정상으로 확인했다(성문 7곳이 폴리곤 위, `coordinate-fixes.json`의 `reviewed` 참고).
